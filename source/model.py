...
from models import mobilenet,resnext,resnet #, mobilenetv2
import pdb
...
def generate_model(opt):
    ...
    if opt.model == 'mobilenet':
        ...
        model=nn.DataParallel(model)  #AGGIUNTO per poter caricare modello allenato articolo
    ...
    if not opt.no_cuda:
        ...
        if opt.pretrain_path:
            ...
            pretrain = torch.load(opt.pretrain_path)   #load on cuda, NO: , map_location=torch.device('cpu')
            # print(opt.arch)
            # print(pretrain['arch'])
        ...
        if opt.pretrain_path:
            ...
            if opt.model in ['mobilenet', 'mobilenetv2', 'shufflenet', 'shufflenetv2']:
                model.module.module.classifier = nn.Sequential(
                    nn.Dropout(0.9),
                    nn.Linear(model.module.module.classifier[1].in_features, opt.n_finetune_classes)
                )
            elif opt.model == 'squeezenet':
                model.module.classifier = nn.Sequential(
                    nn.Dropout(p=0.5),
            ...
            model = modify_kernels(opt, model, opt.modality)
            parameters = get_fine_tuning_parameters(model, opt.ft_portion)  #SOSTITUITO begin_index con portion, errore no_cuda (?)
            return model, parameters
        else:
            model = modify_kernels(opt, model, opt.modality)
...
def _modify_first_conv_layer(base_model, new_kernel_size1, new_filter_num):
    ...
    new_conv = nn.Conv3d(new_filter_num, conv_layer.out_channels, kernel_size=(new_kernel_size1, 7, 7),
                         stride=(1, 2, 2), padding=(1, 3, 3), bias=False).cuda()
