...
import sys
sys.path.insert(1, '/Users/carlu/Desktop/Progetto/GesRec')
...
if __name__ == '__main__':
    ...
    with mp_hands.Hands(static_image_mode=False,
                        ...
                        min_tracking_confidence=0.5) as handDet:

        out = cv2.VideoWriter('result.avi', cv2.VideoWriter_fourcc(*'DIVX'), 15, (270,480))
        i=0
        while cap.isOpened():
            start=time.time()
            ret,frame=cap.read()
            ...
            predictedStr = " Result: "+str(predicted)

            frameShow = cv2.resize(frame, (270,480))
            cv2.putText(frameShow, fps, (0, 15), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (38, 0, 255), 1, cv2.LINE_AA)
            cv2.putText(frameShow, predictedStr, (0, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (38, 0, 255), 1, cv2.LINE_AA)
